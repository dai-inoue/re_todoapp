...
public class Todo implements Comparable<Todo> { 
	...
	public int compareTo(Todo otherTodo) {
		...
		// 「期限が設定されているタスクを優先的に前へ、未設定のタスクを後ろに」というロジックにする
		if(this.time_limit == null && otherTodo.time_limit == null) {
		// 両方がnull場合(どちらともタスク期限が設定されていない場合)
		
		// 現タスクがthis.time_limitがnullで比較対象のタスク (otherTodo) には期限が設定されている
		} else if(this.time_limit == null) {
		// thisがnull期限なしなので、otherTodo(期限あり)よりも「大きい」(ソート順で後)と判断
		// これにより、期限が設定されていないタスクは、期限が設定されているタスクの後に並ぶ
			return 1;
		// 現在のタスク (this) には期限が設定されており、比較対象のタスク (otherTodo) の time_limit が null の場合
		} else if(this.otherTodo.time_limit == null) {
			// otherTodoがnull（期限なし）なので、this（期限あり）よりも「小さい」（ソート順で前）と判断
			// これにより、期限が設定されているタスクは、期限が設定されていないタスクの前に並ぶ
			return -1;
		// 両方のタスクのtime_limit が null でない場合（どちらのタスクも期限が設定されている）
		} else {
